...
// Apagar todas as mensagens do funcionário
if (!empty($payload['all'])) {
    try {
        $stmt = $pdo->prepare('DELETE FROM notificacoes WHERE funcionario_id = ? AND client_id = ?');
        $stmt->execute([$employeeId, $clientId]);
        echo json_encode(['success' => true, 'message' => 'Todas as mensagens foram eliminadas.']);
    } catch (PDOException $e) {
        error_log('delete_sms.php (all) erro: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Erro interno ao eliminar mensagens.']);
    }
    exit;
}
...
try {
    ...
    error_log('delete_sms.php erro: ' . $e->getMessage());
    http_response_code(500);
